...
class SupConLoss(nn.Module):
    ...
    def __init__(self, temperature=0.07, contrast_mode="all", base_temperature=0.07, tree_path=None):
        ...
        if self.tree_path is not None:
            ...
            ### this is to make sure that the tips are sorted alphabetically, and will match the labels from the BioDataset (if the set of tips matches the set of labels)
            ...
            for i, j in combinations_with_replacement(range(n_tips), 2):
                if i == j:
                    pass # we've already set the diagonal to 1.0Add commentMore actions
